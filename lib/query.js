...
class QueryRaw {
  constructor(string = '') {
    if (typeof string !== 'string') {
      throw new TypeError('Query Raw: parameter must be a string')
    }
    this.string = string
  }
...
class QueryString {
  constructor(type = 'any', string = '', contains = true, fuzzy = false) {
    ...
    if (typeof string !== 'string') {
      throw new TypeError('String Type: string must be a string')
    }
    if (typeof contains !== 'boolean' || typeof contains !== 'boolean') {
      throw new TypeError('String Type: contains and fuzzy must be booleans')
    }
...
class QueryMediaType {
  constructor(mediaType = '', is = true) {
    ...
    if (typeof is !== 'boolean') {
      throw new TypeError('Media Type: is must be booleans')
    }
    this.type = mediaType
    this.is = is
  }
...
class QueryRange {
  constructor(type = '', from = undefined, to = undefined, include = true) {
    ...
    if (typeof include !== 'boolean') {
      throw new TypeError('Range: include must be booleans')
    }
    this.type = type
