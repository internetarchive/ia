...
class MetaQueue {
  ...
  loadNext() {
    ...
    return ((this.retryMap[result.name] === 0) ? result.metadata() : new Promise((resolve) => {
      setTimeout(resolve, 10 ** this.retryMap[result.name])
      ...
      .catch((reason) => {
        if (!(
          ...
          this.metaErrorMap[result.identifier] = reason
        }
      })
      .finally(() => {
        this.cb(result)
...
class DownloadQueue extends MetaQueue {
  ...
  downloadNext(resumeMap = { undefined: 0 }) {
    ...
    return ((this.retryMap[file.name] === 0) ? file.download(from) : new Promise((resolve) => {
      setTimeout(resolve, 10 ** this.retryMap[file.name])
      ...
      .catch((reason) => {
        if (!(
          ...
          this.downloadErrorMap[file.name] = reason
        }
      })
