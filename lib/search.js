class Search {
    ...
    constructor(page = 1, rows = 1, end = 100000, query, fields, retry = 5) {
        if (end > this.end) {
    ...
    searchRetry(page = 1, retry = 0) {
        ...
        return this.search(page).catch(
            (reason) => delay(retry).then(
                    () => {
                        if (retry > this.retry) {
                            return new Promise.reject(reason.message)
                        }
        ...
        return  {
            next: () => {
                if (page * this.rows <= this.end) {
                    const result = { value: this.searchRetry(page),  done: false }
                    page++
                    return result
                }
                return {value: this.searchRetry(page), done: true}
            },
            return: () => {
